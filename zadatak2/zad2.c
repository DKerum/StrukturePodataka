...
int main(void) {
    ...
    // Dodaj tri elementa (dva na poèetak i jedan na kraj)
    addToFront(&head);
    addToFront(&head);
    addToEnd(&head);

    // Pronaði osobu po prezimenu
    findByLastName(&head);
...
int printList(Position head) {
    head = head->next; // preskoèi "head" èvor
...
//Pomoæna funkcija: pronalazi prethodnika traženog prezimena
